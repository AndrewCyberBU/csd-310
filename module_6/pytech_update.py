...
# (certain fields), update one document, find the updated document in 
...
# Call the update_one method by student_id 1007 and update the last name to
# Cruz (instead of the original Bush)
db.students.update_one({"student_id": 1007}, {"$set": {"last_name": "Cruz"}})

# Print a message
print("\n-- DISPLAYING STUDENT DOCUMENT 1007 --\n")

# Create a variable named "a_student", find the document with the 1007 student_id,
# and assign it to the "a_student" variable
a_student = students.find_one({"student_id": 1007})

# Print the student_id, first_name, and last_name from the document with the
# 1007 student_id. Student IDs in my database are integers. They are converted to
